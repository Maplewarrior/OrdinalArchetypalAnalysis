...
class _OAA:
    ...
    def _apply_constraints_beta(self, b, c1, c2): 
        m = nn.Softmax(dim=0)
        return self._softplus(c1) * torch.cumsum(m(b), dim=0)[:len(b)] + c2
    ...
    def _calculate_alpha(self, b):
        ...
        alphas = ((b_j_plus1+b_j)/2)[1:-1] # slice due to concat
        alphas[torch.gt(alphas, 1)] = 1.0 # set values greater than one to 1.0
        alphas[torch.lt(alphas, 0)] = 0.0 # set values less than zero to 0.0
        return alphas
    ...
    """
    Not sure whether the tail boundaries should still be -inf and inf or the new 
    values in accordance with the adjustment made to the boundaries?
    
    The below code assumes the new cutoffs are used...
    """
    ...
    def _error(self,Xt,A_non_constraint,B_non_constraint,b_non_constraint,sigma_non_constraint, c1_non_constraint, c2, sigma_cap):

        A = self._apply_constraints_AB(A_non_constraint)
        B = self._apply_constraints_AB(B_non_constraint)
        b = self._apply_constraints_beta(b_non_constraint, c1_non_constraint, c2) ## added c1 and c2
    ...
    def _compute_archetypes(
        ...
        if not for_hotstart_usage:
            ...
            b_non_constraint_np = b_non_constraint.detach().numpy()
            c1_non_constraint_np = c1_non_constraint.detach().numpy()
            c2_np = c2.detach().numpy()
            return A_non_constraint_np, B_non_constraint_np, sigma_non_constraint_np, b_non_constraint_np, c1_non_constraint_np, c2_np
        

    def _compute_archetypes_alternating(
            self, 
            X, 
            K, 
            p, 
            ...
            A_non_constraint = torch.autograd.Variable(torch.tensor(A_np), requires_grad=True)
            B_non_constraint = torch.autograd.Variable(torch.tensor(B_np), requires_grad=True)
            b_non_constraint = torch.autograd.Variable(torch.tensor(b_np), requires_grad=True)
            sigma_non_constraint = torch.autograd.Variable(torch.tensor(sigma_np), requires_grad=True)
            c1_non_constraint = torch.autograd.Variable(torch.tensor(c1_np), requires_grad=True)
            c2 = torch.autograd.Variable(torch.tensor(c2_np), requires_grad=True)
            ...
            for i in range(n_iter):
                ...
                ##############################################
                optimizer.step()
                # print("sigma: ", sigma_non_constraint.detach().numpy())
